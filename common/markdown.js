...
function createReqHeaders (req_headers) {
  ...
  if (req_headers && req_headers.length) {
    ...
    for (let j = 0; j < req_headers.length; j++) {
      headersTable += `| ${req_headers[j].name || ''}  |  ${req_headers[j].value || ''} | ${+req_headers[j].required === 1 ? '是' : '否'
...
function createReqBody (req_body_type, req_body_form, req_body_other, req_body_is_json_schema) {
  if (req_body_type === 'form' && req_body_form.length) {
    ...
    for (let j = 0; j < req_body.length; j++) {
      bodyTable += `| ${req_body[j].name || ''} | ${req_body[j].type || ''}  |  ${+req_body[j].required === 1 ? '是' : '否'
  ...
  columns.map((item, index) => {
    ...
    switch (dataIndex) {
      ...
      case 'desc':
        ...
        break
      case 'name':
        text = `<span style="padding-left: ${20 * level}px"><span style="color: #8c8a8a">${level > 0 ? '├─' : ''
          }</span> ${value}</span>`
        break
      default:
        text = value
